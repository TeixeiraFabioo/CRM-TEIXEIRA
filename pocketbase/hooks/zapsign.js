 ...
 *
 * Endpoints:
 * - POST /api/zapsign/connect
 * - POST /api/zapsign/disconnect
 * - POST /api/zapsign/test
 * - GET  /api/zapsign/config
 * - POST /api/zapsign/create
 * - GET  /api/zapsign/status/{docId}
 * - POST /api/zapsign/webhook
 *
 * Backward-compatibility aliases:
 * - POST /api/signatures/zapsign/connect
 * - POST /api/signatures/zapsign/disconnect
 * - POST /api/signatures/test-connection
 * - GET  /api/signatures/zapsign/config
 * - POST /api/signatures/zapsign/create
 * - GET  /api/signatures/zapsign/status/{docId}
 * - POST /api/webhooks/zapsign
 *
 * Lifecycle hook on contracts: onRecordAfterUpdateSuccess
 */

// --- 1. POST /api/zapsign/connect: Valida e conecta o token ZapSign ---
routerAdd('POST', '/api/zapsign/connect', (e) => {
  try {
    ...
    // Validar token chamando a API do ZapSign
    ...
    return e.json(200, {
      ...
      config: {
        ...
        status: 'active',
        is_active: true,
        updated: configRec.getString('updated') || nowIso,
        created: configRec.getString('created') || nowIso,
      },
    })
  } catch (err) {
    return e.json(500, {
...
// Alias de compatibilidade: POST /api/signatures/zapsign/connect
routerAdd('POST', '/api/signatures/zapsign/connect', (e) => {
  try {
    const reqBody = e.requestInfo().body || {}
    const token = (reqBody.token || reqBody.api_token || '').trim()
    const tenantId = reqBody.tenant_id || (e.auth ? e.auth.get('tenant_id') : '') || ''
...
// --- 2. POST /api/zapsign/disconnect: Remove a integração ---
routerAdd('POST', '/api/zapsign/disconnect', (e) => {
  try {
    const reqBody = e.requestInfo().body || {}
    const tenantId = reqBody.tenant_id || (e.auth ? e.auth.get('tenant_id') : '') || ''
    ...
    try {
      ...
      console.warn('[ZapSign Disconnect] Erro ao deletar config:', delErr)
    }
...
// Alias de compatibilidade: POST /api/signatures/zapsign/disconnect
routerAdd('POST', '/api/signatures/zapsign/disconnect', (e) => {
  try {
    const reqBody = e.requestInfo().body || {}
    const tenantId = reqBody.tenant_id || (e.auth ? e.auth.get('tenant_id') : '') || ''
...
// --- 3. POST /api/zapsign/test: Testa a conexão do token ---
routerAdd('POST', '/api/zapsign/test', (e) => {
  try {
    const reqBody = e.requestInfo().body || {}
    let token = reqBody.token || reqBody.api_token || ''
    const sandbox = !!reqBody.sandbox
    const tenantId = reqBody.tenant_id || (e.auth ? e.auth.get('tenant_id') : '') || ''
...
// Alias de compatibilidade: POST /api/signatures/test-connection
...
// --- 4. GET /api/zapsign/config: Consulta status e config sem expor api_token ---
routerAdd('GET', '/api/zapsign/config', (e) => {
  try {
    const tenantId =
...
// Alias de compatibilidade: GET /api/signatures/zapsign/config
routerAdd('GET', '/api/signatures/zapsign/config', (e) => {
  try {
    const tenantId =
      e.requestInfo().query.tenant_id || (e.auth ? e.auth.get('tenant_id') : '') || ''
...
// --- 5. POST /api/zapsign/create: Criação de documento via API ---
routerAdd('POST', '/api/zapsign/create', (e) => {
  try {
    ...
    let apiToken = body.token || body.api_token || ''
    let isSandbox = false
    let customBaseUrl = ''
    ...
    const docName =
      contractData.titulo || contractData.title || body.name || 'Contrato Comercial - ' + signerName
    ...
    if (signerPhone) {
      ...
      if (cleanPhone) {
        signerObj.phone_number = cleanPhone.startsWith('55') ? cleanPhone.substring(2) : cleanPhone
      }
      ...
      zapsignPayload.markdown =
        ...
        (customerData.address || customerData.city || customerData.state
          ? '**Endereço:** ' +
            ...
            '\n'
          : '') +
        (customerData.estado_civil ? '**Estado Civil:** ' + customerData.estado_civil + '\n' : '') +
        (contractData.valor
          ? '**Valor:** R$ ' +
...
// Alias de compatibilidade: POST /api/signatures/zapsign/create
routerAdd('POST', '/api/signatures/zapsign/create', (e) => {
  try {
    const body = e.requestInfo().body || {}
    const leadData = body.leadData || {}
    const customerData = body.customerData || {}
...
// --- 6. GET /api/zapsign/status/{docId}: Consulta status de documento ZapSign ---
routerAdd('GET', '/api/zapsign/status/{docId}', (e) => {
  try {
    const docId = e.requestInfo().params.docId
    if (!docId) {
      return e.json(400, { success: false, error: 'ID do documento não informado.' })
    }
    ...
    const tenantId =
      (e.auth ? e.auth.get('tenant_id') : '') || e.requestInfo().query.tenant_id || ''
    if (tenantId) {
      try {
        const configs = $app.findRecordsByFilter(
          'integration_configs',
...
// Alias de compatibilidade: GET /api/signatures/zapsign/status/{docId}
routerAdd('GET', '/api/signatures/zapsign/status/{docId}', (e) => {
  try {
    const docId = e.requestInfo().params.docId
    if (!docId) {
      return e.json(400, { success: false, error: 'ID do documento não informado.' })
...
// --- 7. POST /api/zapsign/webhook: Webhook para atualização de status de documentos ---
...
// Alias de compatibilidade: POST /api/webhooks/zapsign
...
// --- 8. Lifecycle hook: onRecordAfterUpdateSuccess em contracts ---
